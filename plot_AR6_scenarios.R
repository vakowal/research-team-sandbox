...
datadir <- "C:/Users/Ginger.Kowal.CORPCDP/Documents/Scenario review/"
outdir <- paste0(datadir, 'outputs/')

# plot gross EIP CO2 from all C1 scenarios, and median of filtered scenarios
co2_df <- read.csv(
  paste0(outdir, 'gross_co2_summary.csv'), check.names=FALSE)
...
plot_df <- pivot_longer(
  co2_df, cols=!c(scen_id, Variable), names_to='year', values_to='CO2')
...
print(p)
filename <- paste0(outdir, "240516_median_filtered_vs_C1.png")
...
# key variables from NZE, and median of filtered scenarios
metric_list <- c(
...
focal_scen <- c("IEA NZE", "Median of filtered scenarios")
keyvar_df$Scenario <- factor(
  keyvar_df$source,
  levels=c('IEA NZE', 'Median of filtered scenarios',
           'AR6 C1 (25th percentile)',
           'AR6 C1 (75th percentile)'),
  labels=c('NZE', 'Filtered', 'AR6 C1-25', 'AR6 C1-75'))
...
print(p)
filename <- paste0(GDRIVE, "20240516_key_variable_summary.png")
